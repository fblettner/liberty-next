...
1. ~~APScheduler 4.0 vs 3.x.~~ **Decided (chunk 2c): APScheduler 3.11.2 with `AsyncIOScheduler`.** As of 13a implementation, 4.0 isn't on PyPI's stable index yet (only pre-releases); 3.11.2 is rock-solid, well-documented, and gives us everything nomaflow needs (cron triggers, timezone support, `max_instances=1`, graceful shutdown). Revisit when 4.0 ships a stable release and a feature we actually need motivates the migration.
