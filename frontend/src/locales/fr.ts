...
const fr: Resources = {
  ...
  table: {
    ...
    grouped: 'groupé',
    export: 'Exporter',
    exportAllColumns: 'Toutes les colonnes (masquées incluses)',
  ...
  import: {
    ...
    drop: 'Déposez un fichier .xlsx / .csv ici, ou cliquez pour parcourir',
    check: 'Vérifier',
    noRows: 'Aucune ligne ne correspond aux colonnes de cet écran.',
    mode: 'Lignes existantes',
    modeUpsert: 'Mettre à jour ou insérer',
    modeInsert: 'Insérer seulement',
    modeUpdate: 'Mettre à jour seulement',
    validating: 'Vérification de {{count}} lignes dans la base…',
    checkingRules: 'Vérification des règles de l’écran…',
    checkingDb: 'Vérification dans la base de données…',
    ruleError: 'règle',
    rowNum: 'Ligne {{n}}',
    ...
    action: 'Action',
    errorCol: 'Erreur',
    saveValid: 'Enregistrer {{count}} lignes valides',
    committedSome: '{{count}} lignes enregistrées.',
  },
  dashboard: {
  ...
  nomaflow: {
    ...
    jobs: {
      ...
      duplicateBtn: 'Cloner',
      duplicateTitle: 'Cloner la tâche',
      duplicatePrompt: 'Nouvel identifiant (lettres, chiffres, - ou _) :',
      badId: 'L\'identifiant ne peut contenir que des lettres, chiffres, - et _.',
      idExists: 'Une tâche nommée « {{id}} » existe déjà.',
      delete: 'Supprimer cette tâche de jobs.toml (configuration uniquement ; l\'historique est conservé).',
      deleteTitle: 'Supprimer la tâche',
      deletePrompt: 'Supprimer « {{id}} » de jobs.toml ? Son planning et sa configuration sont retirés (l\'historique des exécutions est conservé).',
      presetSchedules: '{{count}} planning(s) de préréglage',
      filterByTags: 'Filtre :',
  ...
  settings: {
    ...
    reloadHint: 'Relit tous les fichiers de configuration dans le serveur (application forcée, sans redémarrage).',
    reloadDone: 'Rechargé — {{c}} connecteurs, {{d}} entrées de dictionnaire, {{s}} applications.',
    save: 'Enregistrer',
    reload: 'Recharger',
    unsaved: 'modifications non enregistrées',
    ...
    changes: {
      ...
      include: 'Inclure',
      move: 'Déplacer',
      newPkg: 'Nouveau package',
      newPkgTip: 'Créer un package personnalisé — déplacez-y des changements pour les promouvoir dans un prochain commit',
      newPkgTitle: 'Nouveau package personnalisé',
      newPkgMsg: 'Un package d’attente pour « {{app}} » — déplacez-y des changements pour les promouvoir plus tard. Nom :',
      newPkgPlaceholder: 'ex. Nettoyage des rôles à venir',
      kindCustom: 'perso',
      moveGroup: 'Déplacer cet enregistrement vers un autre package',
      moveEntry: 'Déplacer ce changement vers un autre package',
      deleteGroup: 'Supprimer cet enregistrement du package',
      deleteEntry: 'Supprimer ce changement du package',
      moveTitle: 'Déplacer {{n}} changement(s) vers…',
      moveToNew: 'Nouveau package personnalisé…',
      moveNoTargets: 'Aucun autre package brouillon pour cette application — créez-en un ci-dessous.',
      delEntriesTitle: 'Supprimer le(s) changement(s) ?',
      delEntriesMsg: 'Supprimer définitivement {{n}} changement(s) ({{label}}) de ce package ? Les lignes déjà écrites en base ne sont pas affectées — seul le journal du package est nettoyé, afin qu’une ré-exportation ne puisse pas les réinclure.',
      ...
      postApply: 'post-application',
      postApplyTitle: 'Étapes post-application',
      postApplyHint: 'Une bibliothèque d’étapes exécutées UNE fois, chacune avec un id. Un écran suivi référence la/les étape(s) dont il a besoin via son champ « post-application » (onglet Général). À l’EXPORT, un package n’embarque que les étapes des écrans qui y ont contribué — exécutées une fois sur la cible après toutes les opérations — ex. un écran sécurité tire un remerge ; un écran UDC non. L’alternative groupée à une action change_replay par ligne.',
      paId: 'Id',
      bundleIncludes: '+ {{n}} post-application',
      bundleIncludesTip: 'Les étapes post-application configurées pour ce connecteur sont ajoutées au bundle exporté.',
      paApplication: 'Application',
      paType: 'Type',
      paConnector: 'Connecteur',
      paSameAsApp: '(application)',
      paLabel: 'Libellé',
      paAdd: 'Ajouter une étape',
    ...
    crudWizard: {
      ...
      nameFilter: 'Filtre sur le nom de la table',
      nameFilterPlaceholder: 'ex. F009%, USR_%, % pour toutes',
      typeFilter: 'Saisir un filtre pour lister les tables',
    ...
    generateQuery: {
      ...
      table: 'Table à lire',
      mapColumns: 'Associer chaque paramètre à une colonne',
      pickTableFirst: 'Choisissez une table pour associer ses colonnes.',
      column: 'Colonne…',
      role: { value: 'valeur', label: 'libellé', key: 'clé', max: 'max', param: 'param' },
      queryName: 'Nom de la requête',
      sqlPreview: 'SQL généré — modifiable',
      use: 'Utiliser cette requête',
      pickTable: 'Choisissez une table à lire.',
      nameRequired: 'Nommez la requête.',
      nameExists: 'Une requête nommée « {{name}} » existe déjà.',
      sqlEmpty: 'Le SQL généré est vide.',
      needValue: 'Définissez d’abord la colonne valeur de cette recherche.',
      needDdId: 'Définissez d’abord la colonne (dd_id) de cette séquence.',
    },
    scaffold: {
      table: 'Table',
    ...
    screens: {
      ...
      editor: {
        ...
        importColumnsNone: 'Toutes les colonnes de la requête sont ajoutées',
        importColumnsLoading: 'Importer depuis la requête…',
      ...
      visual: {
        ...
        columnConfig: 'Configuration de la colonne',
        fieldIsColumn: 'Ce champ affiche sa colonne. Modifiez tout — y compris sa largeur dans le dialogue (Affichage ▸ Colspan) — ci-dessous ; la même config pilote la grille.',
        tabSettings: {
          title: 'Paramètres de l\'onglet',
          label: 'Libellé de l\'onglet',
          type: 'Type d\'onglet',
          typeForm: 'Formulaire — une grille de champs éditables',
          typeNestedForm: 'Formulaire imbriqué — réutilise le formulaire d\'un autre écran en ligne (référence un écran)',
    ...
    sqlEditor: {
      ...
      token: 'Jeton',
      tokenTitle: 'Insérer une valeur prédéfinie résolue à l’écriture (liée, convertie au format de la colonne)',
      schemaTokens: 'Schémas du pool',
      tokLogin: 'Utilisateur courant (en majuscules)',
      tokPid: 'Id programme — le nom du connecteur (ex. ULPID)',
      tokJobn: 'Tâche/poste — « LIBERTY » (ex. ULJOBN)',
      tokJdedate: 'Date julienne JDE du jour — entier CYYDDD (ex. ULUPMJ)',
      tokJdetime: 'Heure JDE courante — entier HHMMSS (ex. ULUPMT)',
      tokSysdate: 'Date courante (colonne date native)',
      tokSystime: 'Heure courante (colonne heure native)',
      tokSystimestamp: 'Date et heure courantes',
    },
    sqlWizard: {
      ...
      pickSchema: 'Schéma',
      filterTable: 'Filtrer (ex. F009%)',
      typeFilter: 'Saisir un filtre pour lister les tables',
      addJoin: 'Ajouter une table jointe',
      joinOn: 'ON',
      joinSource: 'Colonne source',
      joinTarget: 'Colonne cible',
      addJoinCond: 'Ajouter une condition',
      joinEditText: 'Éditer en texte',
      joinUseBuilder: 'Mode assisté',
      joinOnExpr: 'ex. a.KEY = b.KEY AND TRIM(b.X) IS NULL',
      readOnlyNote: "La liste SELECT de cette requête utilise des alias de colonnes ou des expressions que l'assistant ne peut pas éditer. Les jointures sont affichées ci-dessus à titre indicatif — éditez la requête directement en SQL.",
      column: 'colonne',
      columns: '{{n}} sur {{total}} colonnes sélectionnées',
      columnsTitle: 'Colonnes',
      title_post: 'Construire un INSERT',
      title_put: 'Construire un UPDATE',
      title_delete: 'Construire un DELETE',
      changeTable: 'Changer de table',
      insertCols: 'Colonnes à insérer',
      updateCols: 'Colonnes à mettre à jour · les clés pilotent le WHERE',
      deleteCols: 'Colonnes clés (pilotent le WHERE)',
      key: 'clé',
      notSelectNote: "Ce n'est pas un SELECT simple que l'assistant peut éditer — éditez-le directement en SQL.",
      notMatchNote: "Cette instruction ne commence pas par {{kw}} — éditez-la directement en SQL.",
      where: 'Où',
