...
const en = {
  ...
  profile: {
    ...
    passwordOidc: 'Your password is managed by your identity provider.',
  ...
  table: {
    ...
    grouped: 'grouped',
    export: 'Export',
    exportAllColumns: 'All columns (incl. hidden)',
  ...
  import: {
    ...
    drop: 'Drop a .xlsx / .csv here, or click to browse',
    check: 'Check',
    noRows: 'No rows matched this screen’s columns.',
    mode: 'Existing rows',
    modeUpsert: 'Update or insert',
    modeInsert: 'Insert only',
    modeUpdate: 'Update only',
    validating: 'Checking {{count}} rows against the database…',
    checkingRules: 'Checking screen rules…',
    checkingDb: 'Checking against the database…',
    ruleError: 'rule',
    rowNum: 'Row {{n}}',
    saving: 'Saving…',
    total: '{{count}} rows',
    ...
    action: 'Action',
    errorCol: 'Error',
    saveValid: 'Save {{count}} valid rows',
    committedSome: '{{count}} rows saved.',
  },
  dashboard: {
  ...
  nomaflow: {
    ...
    jobs: {
      ...
      duplicate: 'Clone this job (config copied to a new id; disabled + unscheduled).',
      duplicateBtn: 'Clone',
      duplicateTitle: 'Duplicate job',
      duplicatePrompt: 'New job id (letters, digits, - or _):',
      badId: 'Job id may only contain letters, digits, - and _.',
      idExists: 'A job named "{{id}}" already exists.',
      delete: 'Delete this job from jobs.toml (config only; run history kept).',
      deleteTitle: 'Delete job',
      deletePrompt: 'Delete "{{id}}" from jobs.toml? Its schedule and config are removed (past run history is kept).',
      presetSchedules: '{{count}} preset schedule(s)',
      filterByTags: 'Filter:',
      clearFilters: 'Clear',
    },
    editor: {
  ...
  settings: {
    ...
    reloading: 'Reloading…',
    reloadHint: 'Re-read every config file into the running server (force-apply, no restart).',
    reloadDone: 'Reloaded — {{c}} connectors, {{d}} dictionary entries, {{s}} apps.',
    save: 'Save',
    reload: 'Reload',
    unsaved: 'unsaved changes',
    ...
    changes: {
      ...
      move: 'Move',
      newPkg: 'New package',
      newPkgTip: 'Create a custom holding package — move changes into it to promote in a later commit',
      newPkgTitle: 'New custom package',
      newPkgMsg: 'A holding package for "{{app}}" — move changes into it to promote later. Name:',
      newPkgPlaceholder: 'e.g. Future role cleanup',
      kindCustom: 'custom',
      moveGroup: 'Move this record to another package',
      moveEntry: 'Move this change to another package',
      deleteGroup: 'Delete this record from the package',
      deleteEntry: 'Delete this change from the package',
      moveTitle: 'Move {{n}} change(s) to…',
      moveToNew: 'New custom package…',
      moveNoTargets: 'No other draft package for this application yet — create one below.',
      delEntriesTitle: 'Delete change(s)?',
      delEntriesMsg: 'Permanently remove {{n}} change(s) ({{label}}) from this package? The rows already written to the database are not affected — only the package log is cleaned up, so a re-export can’t re-include them.',
      export: 'Export bundle',
      applyBundle: 'Apply bundle…',
      applyTitle: 'Apply a promotion bundle',
      badBundle: 'Not a valid JSON bundle.',
      nOps: 'op(s)',
      dryRun: 'Dry run',
      dryRunReport: 'Dry run — nothing written. ',
      applied: 'Applied. ',
      apply: 'Apply',
      ...
      postApplyTitle: 'Post-apply steps',
      postApplyHint: 'A library of run-once steps, each with an id. A change-tracked screen references the step(s) it needs via its “post-apply” field (General tab). On EXPORT, a package carries only the steps of the screens that contributed to it — run once on the target after every change op — e.g. a security screen pulls in a remerge; a UDC screen doesn’t. The batch alternative to a per-row change_replay action.',
      paId: 'Id',
      bundleIncludes: '+ {{n}} post-apply',
      bundleIncludesTip: 'Run-once post-apply steps configured for this connector are appended to the exported bundle.',
    ...
    templates: {
      ...
      add: 'New template',
      addTitle: 'New custom report',
      editTitle: 'Edit custom report',
      empty: 'No custom report templates yet. Click "New template" to author one — operators with the reports:custom:*:run permission will see it appear under /reports.',
      count_one: '{{count}} template',
      count_other: '{{count}} templates',
      noParams: 'No parameters declared. Add one for each input the report needs at run time.',
      addParam: 'Add parameter',
      saved: 'Saved. {{count}} template(s) live — the registry refreshed; no restart needed.',
      confirmDeleteTitle: 'Delete template?',
      confirmDelete: 'Delete template "{{id}}"? This rewrites reports.toml and refreshes the report registry.',
      idExists: 'A template with id "{{id}}" already exists.',
      templateHint: 'Jinja markdown. Context: ctx.data.rows (list of row dicts), ctx.data.columns, ctx.params. Sandboxed — Python builtins / dunder access blocked.',
      testParams: 'Test parameters',
      testParamPlaceholder: '(no default)',
      testRender: 'Test render',
      testRenderTitle: 'Run the bound query and render the template — preview only, nothing is saved.',
      testRenderHint: 'Runs the bound query with the test parameters above and renders the template, capped at 200 rows.',
      previewMeta: 'Rendered {{rendered}} of {{total}} rows',
      previewTruncated: 'Showing the first {{rendered}} of {{total}} rows. The full PDF will include every row.',
      previewFailed: 'Render failed',
      ...
      examples: {
        ...
        insertTitle: 'Replace the editor body with this starter template.',
        replaceTitle: 'Replace the template body?',
        replaceConfirm: 'Your edits to the template body will be discarded.',
        replaceConfirmLabel: 'Replace',
      },
      scaffold: 'Scaffold from query',
      scaffoldTitle: 'Emit a markdown table using the {{n}} columns the bound query returns.',
      scaffoldDisabled: 'Pick a connector and query first — the scaffold uses the live query columns.',
      columnsError: 'Could not introspect the query — autocomplete unavailable',
      fields: {
        id: 'Slug (URL segment)',
        title: 'Title',
        ...
        query: 'Named query',
        params: 'Parameters',
        templateBody: 'Jinja markdown template',
        required: 'required',
      },
      errors: {
        idRequired: 'Slug is required.',
        idShape: 'Slug must be kebab-case: lowercase letters, digits, dashes, underscores.',
        titleRequired: 'Title is required.',
        dataRequired: 'Pick a connector and a query.',
        templateRequired: 'Template body is required.',
        dupParam: 'Two parameters share the same name — rename one.',
      },
    },
    reports: {
    ...
    theme: {
      ...
      primaryHint: "Leave blank to use the preset's colour.",
      appName: 'App name',
      font: 'Font family',
      fontHint: 'The UI font for the whole install. Code stays monospace.',
      size: 'Text size',
      sizeCompact: 'Compact',
      sizeNormal: 'Normal',
      ...
      sizeLarge: 'Large',
      reset: 'Reset',
      saved: 'Saved. The new theme is live for everyone.',
      previewHint: 'Live preview — buttons, links and active states use the primary accent.',
    ...
    connectors: {
      ...
      name: 'Name',
      clone: 'Clone',
      cloneSource: 'Source connector',
      cloneNewApp: 'New name',
      cloneNewPool: 'Pool',
      confirmDeleteApp: 'Delete connector {{name}} and everything that belongs to it?',
      deleteAppSuccess: 'Deleted connector {{name}}.',
      deleteQuery: 'Delete query',
      confirmDeleteQuery: 'Delete query {{name}}?',
      ...
      changeTypePrompt: 'Move "{{name}}" to which kind?',
      changeTypeMultiSlot: 'This table has more than one CRUD slot, so it is a real CRUD set — not a single query. Delete the extra slots first if it is really just one query.',
      changeTypeRenameWarn: 'This changes the query\'s runtime name from "{{old}}" to "{{new}}". Update any screen / action / dictionary reference to the new name afterwards. Continue?',
      addQuery: 'Add query',
      queryNamePrompt: 'Query name (e.g. post_invoice, customer_balance):',
      emptyCustom: 'No unclassified queries. These are queries not tied to a table’s get/put/post/delete set and not used as a sequence or lookup — click "Add query" to create one.',
      ...
      renameQueryPrompt: 'Rename query “{{name}}” to (updates every reference):',
      renameTable: 'Rename table',
      renameTablePrompt: 'Rename table “{{name}}” to (renames all CRUD slots + screen bindings):',
      moveButton: 'Move',
      moveTitle: 'Move {{kind}} "{{name}}"',
      moveTo: 'Move to connector',
      moveDone: 'Moved {{name}} to {{to}}. {{refs}} reference(s) rewritten.',
      moveManual: 'Fix these by hand (connector shared with queries left behind):',
      moveNoTarget: 'No other SQL connector to move to — create one in the Connectors tab first.',
    ...
    crudWizard: {
      ...
      nameFilter: 'Table name filter',
      nameFilterPlaceholder: 'e.g. F009%, USR_%, % for all',
      typeFilter: 'Type a filter to list tables',
    ...
    generateQuery: {
      ...
      mapColumns: 'Map each parameter to a column',
      pickTableFirst: 'Pick a table to map its columns.',
      column: 'Column…',
      role: { value: 'value', label: 'label', key: 'key', max: 'max', param: 'param' },
      queryName: 'Query name',
      sqlPreview: 'Generated SQL — editable',
      use: 'Use this query',
      pickTable: 'Pick a table to read from.',
      nameRequired: 'Name the query.',
      nameExists: 'A query named "{{name}}" already exists.',
      sqlEmpty: 'The generated SQL is empty.',
      needValue: 'Set this lookup’s value column first.',
      needDdId: 'Set this sequence’s column (dd_id) first.',
    },
    scaffold: {
      table: 'Table',
    ...
    screens: {
      ...
      editor: {
        ...
        importColumnsNone: 'All read-query columns added',
        importColumnsLoading: 'Import from read query…',
        ...
        queries: {
          ...
          fromTable: 'Fill from table',
          fromTableHint: 'Pick a table to fill all four CRUD queries…',
        },
      ...
      visual: {
        ...
        fieldIsColumn: 'This field renders its column. Edit everything — including its dialog span (Display ▸ Colspan) — below; the same config drives the grid.',
        tabSettings: {
          title: 'Tab settings',
          label: 'Tab label',
          type: 'Tab type',
          typeForm: 'Form — a grid of editable fields',
          typeNestedForm: 'Nested form — reuse another screen\'s form inline (references a screen)',
      ...
      action: {
        ...
        navTargetType: 'Target type',
        navTargetQuery: 'Query',
        navTargetScreen: 'Screen',
    ...
    tables: {
      ...
      duplicate: 'Clone',
      duplicatePrompt: 'Name of the new copy:',
    ...
    sqlEditor: {
      ...
      token: 'Token',
      tokenTitle: 'Insert a predefined value resolved at write time (bound, format-coerced to the column)',
      schemaTokens: 'Pool schemas',
      tokLogin: 'Current user (uppercased)',
      tokPid: 'Program id — the connector name (e.g. ULPID)',
      tokJobn: 'Job/workstation — "LIBERTY" (e.g. ULJOBN)',
      tokJdedate: 'JDE Julian date today — CYYDDD integer (e.g. ULUPMJ)',
      tokJdetime: 'JDE time now — HHMMSS integer (e.g. ULUPMT)',
      tokSysdate: 'Current date (native date column)',
      tokSystime: 'Current time (native time column)',
      tokSystimestamp: 'Current date & time',
    },
    sqlWizard: {
      ...
      pickSchema: 'Schema',
      filterTable: 'Filter (e.g. F009%)',
      typeFilter: 'Type a filter to list tables',
      addJoin: 'Add a joined table',
      joinOn: 'ON',
      joinSource: 'Source column',
      joinTarget: 'Target column',
      addJoinCond: 'Add condition',
      joinEditText: 'Edit as text',
      joinUseBuilder: 'Use builder',
      joinOnExpr: 'e.g. a.KEY = b.KEY AND TRIM(b.X) IS NULL',
      readOnlyNote: "This query's SELECT list uses column aliases or expressions the builder can't edit. The joins are shown above for reference — edit the query directly in SQL.",
      column: 'column',
      columns: '{{n}} of {{total}} columns selected',
      columnsTitle: 'Columns',
      title_post: 'Build an INSERT',
      title_put: 'Build an UPDATE',
      title_delete: 'Build a DELETE',
      changeTable: 'Change table',
      insertCols: 'Columns to insert',
      updateCols: 'Columns to set · key columns drive WHERE',
      deleteCols: 'Key columns (drive WHERE)',
      key: 'key',
      notSelectNote: "This isn't a simple SELECT the builder can edit — edit it directly as SQL.",
      notMatchNote: "This statement doesn't start with {{kw}} — edit it directly as SQL.",
      where: 'Where',
