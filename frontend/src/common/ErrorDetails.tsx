...
/** Pull the SQLAlchemy ``[SQL: …] [parameters: …]`` tail apart so it can be shown readably — SQL on
 *  its own, and one bind per line. The params is a Python dict/tuple repr: a new key always follows
 *  ``, '`` / ``, "`` while inner commas of values (e.g. ``datetime.datetime(2026, 6, 15)``) are
 *  followed by a digit, so splitting before a quote keeps values intact. Best-effort: returns nulls
 *  when the shape isn't recognised (caller then shows the raw text). */
function parseDbError(message: string): { sql: string | null; params: string | null } {
  const full = (message || '').trim()
  const m = /\[SQL:\s*([\s\S]*?)\]\s*\[parameters:\s*([\s\S]*?)\]\s*(?:\(Background[\s\S]*)?$/.exec(full)
  if (m) return { sql: m[1].trim(), params: m[2].trim().replace(/,\s*(?=['"])/g, ',\n') }
  const ms = /\[SQL:\s*([\s\S]*?)\]\s*$/.exec(full)
  if (ms) return { sql: ms[1].trim(), params: null }
...
const SubLabel = styled.div`
  font-size: ${fontSize.micro}; text-transform: uppercase; letter-spacing: 0.05em;
  color: ${colors.text.muted}; font-family: ${fonts.sans};
`
const Block = styled.pre`
  margin: 0; width: 100%; box-sizing: border-box; max-height: 200px; overflow: auto;
 ...
 *  parameter dump (or otherwise differs), a "Show details" toggle. Details render the SQL and the
 *  bound parameters as separate, readable blocks (one bind per line); falls back to the raw text
 *  when the shape isn't a recognised SQLAlchemy error. Use anywhere a raw backend error would
 *  otherwise overflow a Banner. */
export function ErrorDetails(
  ...
  const { sql, params } = useMemo(() => parseDbError(message), [message])
  const structured = !!(sql || params)
  return (
      ...
      {open && hasDetails && (
        structured ? (
          <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 6 }}>
            {sql && <><SubLabel>{t('common.sql', 'SQL')}</SubLabel><Block>{sql}</Block></>}
            {params && <><SubLabel>{t('common.parameters', 'Parameters')}</SubLabel><Block>{params}</Block></>}
          </div>
        ) : (
          <Block>{message}</Block>
        )
